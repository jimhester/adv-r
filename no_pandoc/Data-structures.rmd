...
Set or get these attributes with their accessor functions: use `names(x)`, `class(x)` and `dim(x)`, not `attr(x, "names")`, `attr(x, "class")`, and `attr(x, "dim")`.
...
Names should be unique, but this restriction is not enforced. However, if names are not unique, character subsetting (see [subsetting](Subsetting.html)), will only return the first match.
