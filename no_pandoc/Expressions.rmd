...
* __calls__ represent the action of calling a function, not its result. These are suffixed with `()`.  The arguments to the function are listed below it, and can be constants, names or other calls.
    ...
    As mentioned in [Functions](Functions.html), even things that don't look like function calls still follow this same hierarchical structure:
...
Note that the second example produces the name ` `a b` `: the backticks are the standard way of escape non-standard names in R. (And are explained more in [Functions](Functions.html))
...
`as.call()` is a minor variation that takes a list where the first argument is the _name_ of a function (not a string), and the subsequent values are the arguments. 
